...
function fullSyncUsers({client, dataStore}) {
    ...
    do {
        ...
        users.map(user => console.log(user.name, user.name, user.email, user.sys_domain_path, user.name, user.sys_id))
        dataStore.save(User.tableModel, users.map(user =>
            new User(user.name, user.name, user.email, user.sys_domain_path, user.name, user.sys_id)
        ))
        offset = offset + 100
...
function fullSyncGroups({client, dataStore}) {
    ...
    do {
        ...
        groups.map(group => console.log(group.name, group.name, "domain", group.name, group.sys_id, group.parent.value))
        dataStore.save(Group.tableModel, groups.map(group =>
            new Group(group.name, group.name, "domain", group.name, group.sys_id, group.parent.value)
        ))
        offset = offset + 100
        console.log(`offset: ${offset}`)
    } while (offset < 400)
